...
void refine_with_pattern(){
	...
	int hasDetail[3];
	hasDetail[0]=int(prepareVert(9) && mustTess[0]<130);
	hasDetail[1]=int(prepareVert(0) && mustTess[1]<130);
	hasDetail[2]=int(prepareVert(3) && mustTess[2]<130);
	int index = (hasDetail[0] << 0) | 
				(hasDetail[1] << 1) | 
				(hasDetail[2] << 2);
	// Form the triangles
	switch(index){
