	...
	if (factor > .001){
		...
		switch(t)
		{
			case 0:
				...
				vec2 parallaxTexcoords = tc;
				float ph = (texture(detail0, parallaxTexcoords).r * parallaxScale) + parallaxBias;
				parallaxTexcoords += ph * viewVec.xy;

				vec2 pdC	= vec2(0.0, 0.0);//texture(pdmap, frag_TexCoord).rg * 2.0 - 1.0;
				vec2 pdN	= (texture(detail0N, parallaxTexcoords).rg * 2.0 - 1.0) * 1.0;
				vec3 parN	= vec3(pdC.r + pdN.r, 1.0, pdC.g + pdN.g);
				
				normal = normalize(mat3(view) * parN);
