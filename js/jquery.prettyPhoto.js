/* ------------------------------------------------------------------------
	...
	Version: 3.0
	Modified by: Casar Davorin for use on MCA
------------------------------------------------------------------------- */
...
(function($) {
	...
	$.fn.prettyPhoto = function(pp_settings) {
		pp_settings = jQuery.extend({
			...
			// To get rid of some problems by viewing myspace,yahoo,foxsearchlight,spike,metacafe,dailymotion,cnn video we use only embed element without object
			...
			$pp_pic_holder.fadeIn(function(){
				...
				switch(_getFileType(pp_images[setPosition])){
					...
					case 'myspace':
                        ...
                        if(settings.autoplay) autoplay = ",ap=1";
						movie = 'http://mediaservices.myspace.com/services/media/embed.aspx/m='+grab_param('videoid',pp_images[setPosition])+',t=1,mt=video'+autoplay;
					
						toInject = settings.other_markup.replace(/{width}/g,correctSizes['width']).replace(/{height}/g,correctSizes['height']).replace(/{wmode}/g,settings.wmode).replace(/{path}/g,movie);
					break;
					
					case 'facebook':
						correctSizes = _fitToViewport(movie_width,movie_height); // Fit item to viewport

						movie = 'http://facebook.com/v/'+grab_param('v',pp_images[setPosition]);
						if(settings.autoplay) movie += "&autoplay=1";
					...
					case 'google':
						...
						movie = 'http://video.google.com/googleplayer.swf?docId='+grab_param('docid',pp_images[setPosition]);
						if(settings.autoplay) movie += "&autoplay=1";
					...
					case 'yahoo':
						...
						videoid = pp_images[setPosition];
						video_ids = videoid.replace('http://video.yahoo.com/watch/','');
						var video_Id = video_ids.split('/');
                        video_ID = video_Id[1];
						if(settings.autoplay) autoplay = "autoplay=1";

						movie_id = grab_param('/(?:watch\/)([a-zA-Z0-9_]+)/',pp_images[setPosition])		
						movie = 'http://d.yimg.com/static.video.yahoo.com/yep/YV_YEP.swf?ver=2.2.46&amp;id='+video_ID+'&amp;vid='+movie_id;
						
						toInject =  settings.other_markup.replace(/{width}/g,correctSizes['width']).replace(/{height}/g,correctSizes['height']).replace(/{wmode}/g,settings.wmode).replace(/{path}/g,movie).replace(/{flashvars}/g,autoplay);
					break;
					
					case 'foxsearchlight':
						correctSizes = _fitToViewport(movie_width,movie_height); // Fit item to viewport

						movie = grab_param('bctid',pp_images[setPosition]);
						if(settings.autoplay) autoplay = "true";
					
						toInject = settings.other_markup.replace(/{width}/g,correctSizes['width']).replace(/{height}/g,correctSizes['height']).replace(/{wmode}/g,settings.wmode).replace(/{path}/g,'http://c.brightcove.com/services/viewer/federated_f8/1138370309').replace(/{flashvars}/g,'videoId='+movie+'&playerId=1138370309&viewerSecureGatewayURL=https://console.brightcove.com/services/amfgateway&servicesURL=http://services.brightcove.com/services&cdnURL=http://admin.brightcove.com&domain=embed&autoStart='+autoplay+'&');
					break;		
					...
					case 'spike':
						correctSizes = _fitToViewport(movie_width,movie_height); // Fit item to viewport
					   	...
					   	videoid = pp_images[setPosition];
						video_ids = videoid.replace('http://www.spike.com/video/','');
						var video_Id = video_ids.split('/');
                        video_ID = video_Id[1];

						movie = 'http://media.mtvnservices.com/mgid:ifilm:video:spike.com:'+video_ID;
						if(settings.autoplay) movie += "&autoplay=1";
					...
					case 'cnn':
						...
						movie = 'http://i.cdn.turner.com/cnn/.element/apps/cvp/3.0/swf/cnn_416x234_embed.swf?context=embed_edition&amp;videoId='+ movie_id.replace('http://cnn.com/video/?/video/','');
						if(settings.autoplay) movie += "&autoplay=1";
					...
					case 'metacafe': 								
						...
						var movie_id = pp_images[setPosition];
						...
						toInject =  settings.other_markup.replace(/{width}/g,correctSizes['width']).replace(/{height}/g,correctSizes['height']).replace(/{wmode}/g,settings.wmode).replace(/{path}/g,movie).replace(/{flashvars}/g,'playerVars='+autoplay);
					break;
					...
					case 'dailymotion':
					    ...
					    movie_id = pp_images[setPosition];
						movie = 'http://dailymotion.com/swf/'+ movie_id.replace('http://dailymotion.com/video/','');
						if(settings.autoplay) movie += "&autoplay=1";
					...
					case 'wm':
						...
						toInject = settings.wm_markup.replace(/{width}/g,correctSizes['width']).replace(/{height}/g,correctSizes['height']).replace(/{wmode}/g,settings.wmode).replace(/{path}/g,filename).replace(/{autoplay}/g,settings.autoplay);
					break;
					...
					case 'local':
						...
						toInject =  settings.flash_markup.replace(/{width}/g,correctSizes['width']).replace(/{height}/g,correctSizes['height']).replace(/{wmode}/g,settings.wmode).replace(/{path}/g,'player.swf?file='+videoPath+'&amp;autostart=1&amp;backcolor=0x000000&amp;frontcolor=0xFFFFFF&amp;lightcolor=0xCC0000&amp;width=500&amp;height=340');
					break;
		...
		*/
		...
		function _getFileType(itemSrc){
			if (itemSrc.match(/youtube\.com\/watch/i)) {
			...
			}else if (itemSrc.match(/yahoo\.com\/watch/i)) {
				return 'yahoo';
				...
				return 'foxsearchlight';
				
			}else if (itemSrc.match(/spike\.com\/video/i)) {
				return 'spike';
			...
			}else if (itemSrc.match(/myspace\.com/i)) {
				return 'myspace';
				
			}else if(itemSrc.indexOf('.mov') != -1){ 
				return 'quicktime';
				
			}else if((itemSrc.indexOf('.wmv') != -1)||(itemSrc.indexOf('.wma') != -1)||(itemSrc.indexOf('.asx') != -1)){
				return 'wm';
